...
(function () {
  ...
  window.addEventListener("pointerup", function (e) {
    ...
    if (connecting) {
      ...
      removeTempEdge();
      nodeLayer.querySelectorAll(".node.connect-target").forEach(function (x) { x.classList.remove("connect-target"); });
      if (targetEl && targetEl.dataset.uid !== fromUid) {
        var toUid = targetEl.dataset.uid;
        var dup = state.edges.some(function (x) { return x.from === fromUid && x.to === toUid; });
        if (!dup) {
          pushHist();
  ...
  document.getElementById("btnX").addEventListener("click", function () {
    if (!state.nodes.length) { alert("共有する構成図がありません。"); return; }
    var text = PNAME + " の構成図を CLOUDCERT_ の構成図ビルダーで描いてみた（リンク先で開いて編集できます）";
    window.open("https://twitter.com/intent/tweet?text=" + encodeURIComponent(text) +
      "&url=" + encodeURIComponent(shareUrl()), "_blank", "noopener");
  });
