...
window.CERT_EXAMS["gcp-ace"] = {
  ...
  "questions": [
    ...
    },
    {
      ...
      "explanation": "請求先アカウントは複数のプロジェクトに紐付け可能であり（選択肢B）、これにより一元的なコスト管理が実現します。また、請求先アカウントは Google Cloud 組織に関連付けることができ、roles/billing.admin を持つ組織管理者が管理できます（選択肢D）。選択肢Aは誤りで、1 つの請求先アカウントで多数のプロジェクトを管理できます。選択肢Cは誤りで、1 つのプロジェクトが同時に複数の請求先アカウントに紐付けることはできません。プロジェクトは常に 1 つの請求先アカウントにのみ紐付けられます。"
    },
    {
      "id": "q015",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "Cloud Spanner（選択肢B）はグローバルに分散しながら強整合性を維持できる唯一のリレーショナルデータベースサービスで、水平スケーリングが可能です。Firestore（選択肢D）はサーバーレスのドキュメントデータベースで、リアルタイムリスナーによるデータ同期をネイティブでサポートしています。Cloud Storage（選択肢A）はオブジェクトストレージで、構造化データのリレーショナルクエリには使用しません。Cloud Bigtable（選択肢C）はペタバイトスケールの大量データ処理に最適化されており、数百ギガバイト以上のデータで真価を発揮します。小規模データセットには Firestore や Cloud SQL の方が適しています。"
    },
    {
      "id": "q032",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "正しいコマンドは gcloud compute instances create <NAME> --machine-type=<TYPE> --zone=<ZONE> です。gcloud compute が Compute Engine のサブコマンドグループで、instances create が操作です。選択肢B の gcloud instances create は存在しません（compute サブコマンドが必要）。選択肢C の gcloud compute create instance は引数の順序が誤りで、正しくは gcloud compute instances create です。また、VM はゾーンリソースのため --zone で指定します（--region は使いません）。選択肢D の gcloud vm create も存在しないコマンドです。"
    },
    {
      "id": "q038",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "GKE Autopilot はノードレベルのインフラ管理（プロビジョニング・パッチ適用・スケーリング・アップグレード）をすべて Google が担当し、ユーザーは Pod の定義に集中できます。課金も Pod が要求する CPU/メモリに基づく Pod 単位課金になります。選択肢B は誤りで、Autopilot ではカスタムノードプールの設定はできません（ノード管理は Google に委ねられます）。選択肢C は誤りで、GKE Standard でも Horizontal Pod Autoscaler（HPA）と Cluster Autoscaler で自動スケーリングできます。選択肢D は誤りで、常時高稼働の本番ワークロードでは GKE Standard の方がコスト効率が高い場合もあります。"
    },
    {
      "id": "q044",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "Cloud Functions 第 2 世代の最大タイムアウトは 60 分で、HTTP トリガーおよびイベントトリガー（Pub/Sub を含む）の両方に適用されます。30 分の処理であれば Cloud Functions 第 2 世代のタイムアウト内に収まります。第 1 世代の最大タイムアウトは 9 分であり、30 分の処理には不十分でした。選択肢B はトリガータイプ別の誤った情報です。Cloud Functions にはタイムアウト制限があります（選択肢D は誤り）。なお、60 分を超える長時間処理が必要な場合は Cloud Run Jobs（最大 24 時間）の使用を検討すべきです。"
    },
    {
      "id": "q046",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "正しいコマンドは gcloud run deploy <SERVICE_NAME> --image=<IMAGE_URL> --region=<REGION> です。Artifact Registry のイメージ URL は <REGION>-docker.pkg.dev/<PROJECT_ID>/<REPO>/<IMAGE>:<TAG> の形式になります。選択肢B の gcloud cloud-run は存在しないコマンドグループです（gcloud run が正しいです）。選択肢C の gcloud container run も存在しません（container は GKE 関連のサブコマンドで、run とは別グループです）。選択肢D の gcloud run create は Cloud Run のサービス作成とは異なります（create サブコマンドは Cloud Run では使いません、deploy を使います）。"
    },
    {
      "id": "q049",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "GPU を使用する Compute Engine インスタンスの作成には --accelerator=type=<GPU_TYPE>,count=<N> フラグを使います。また、GPU を使用するインスタンスは --maintenance-policy=TERMINATE の設定が必要です（ライブマイグレーション非対応のため）。インスタンスの再起動が必要な場合は --restart-on-failure=true（デフォルト）を設定します。選択肢B の --gpu-type と --gpu-count フラグは存在しません。選択肢C の --accelerator=nvidia-t4 は GPU タイプの指定形式が誤りで、nvidia-tesla-t4 が正しい識別子です。選択肢D の --add-gpu フラグも存在しません。"
    },
    {
      "id": "q061",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "Compute Engine のスナップショットはディスクに対して作成します。gcloud compute disks snapshot <DISK_NAME> --snapshot-names=<SNAPSHOT_NAME> --zone=<ZONE> が正しい構文です。選択肢B の gcloud compute snapshots create も有効な代替コマンドですが、--source-disk と --zone が必要で、選択肢A の形式の方が一般的に使用されます。選択肢C の gcloud compute instances snapshot は存在しないサブコマンドです（スナップショットはインスタンスではなくディスクに対して行います）。選択肢D の gcloud storage snapshots は Cloud Storage のコマンドグループで、Compute Engine ディスクとは無関係です。"
    },
    {
      "id": "q064",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "可用性 SLI として HTTP 成功率（エラー率の逆数）は最も基本的な指標で、ユーザーが正常にサービスを利用できているかを測ります（選択肢A）。レイテンシ SLI としてリクエストの p99 応答時間はユーザー体験を反映する重要な指標です（選択肢C）。Cloud Billing の費用（選択肢B）はビジネス指標ではありますが、SLI（サービスの品質指標）としては使いません。VM の起動回数（選択肢D）は運用イベントの記録ですが、ユーザーが体験するサービス品質の指標ではありません。"
    },
    {
      "id": "q079",
      "type": "single",
    ...
    },
    {
      ...
      "choices": [
        ...
        "Cloud Monitoring のワークスペース（スコーピングプロジェクト）",
        "Cloud Trace のサービス設定",
    ...
    },
    {
      ...
      "explanation": "OS Login を有効にすると、VM へのSSHアクセスが Google アカウントの IAM（roles/compute.osLogin または roles/compute.osAdminLogin）で管理されます。手動での SSH キー管理が不要になり、ユーザーが組織を離れた際もすぐにアクセスを無効化できます。2FA との統合も可能です。静的 SSH キーのメタデータ管理は鍵の配布・ローテーション・失効管理が煩雑です。VPN 経由の制限はネットワーク経路の制限であり、認証管理とは別の概念です。IAP TCP 転送（選択肢B）も安全なアクセス方法ですが、OS Login と組み合わせて使うことが一般的です。OS Login が IAM ベースの鍵管理として最も直接的な回答です。"
    },
    {
      "id": "q098",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "サービスアカウントキーを使用しない（Workload Identity・ADC を活用する）ことで、キーの漏洩リスクとローテーション管理の負担を排除できます（選択肢B）。また、コンポーネントごとに専用のサービスアカウントを作成して最小権限を付与すると、1 つのサービスアカウントが侵害されても他への影響を最小化できます（選択肢D）。roles/owner を付与するのは最小権限の原則に完全に反します（選択肢A）。複数アプリでのサービスアカウント共用はどのアプリがどのリソースにアクセスしているか追跡できなくなり、1 つのアプリの侵害が他のアプリにも影響します（選択肢C）。"
    },
    {
      "id": "q100",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "Google Cloud VPC のデフォルトの暗黙ルールでは、外部からのすべてのインバウンドトラフィック（ingress）が優先度 65534 でブロックされます（deny all ingress）。内部（同一 VPC）からのトラフィックは別のルールで許可されます。新規作成プロジェクトの「default」VPC ネットワークには、SSH・RDP・ICMP などを許可するデフォルトファイアウォールルールがプリセットされていますが、これはプリセットルールで、暗黙のデフォルトはすべてブロックです。選択肢B・C・D はすべて誤りで、VPC のデフォルト動作はインバウンドをすべてブロックすることです。"
    },
    {
      "id": "q102",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "IAM 条件は CEL（Common Expression Language）を使ってアクセスに条件を追加します。時間ベースの条件（request.time < timestamp('2024-12-31T23:59:59Z') など）で特定時間帯のみアクセスを許可できます（選択肢A）。また、resource.name や resource.type などのリソース属性に基づいて、特定のリソースへのアクセスのみを許可できます（選択肢C）。サービスアカウントへのロールの種類の制限は IAM 条件ではなく組織ポリシーで管理します（選択肢B）。IAM ロールの権限セット変更はカスタムロールの作成・編集で行うもので、IAM 条件の機能ではありません（選択肢D）。"
    },
    {
      "id": "q110",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "Cloud Monitoring のアップタイムチェックでは、監視対象のエンドポイント（URL・IP・GCE インスタンス・App Engine サービスなど）の指定（選択肢A）と、チェック間隔（1分・5分等）および失敗時の通知チャネル（メール・PagerDuty等）の設定（選択肢C）が必須です。アップタイムチェックはエンドポイントへの HTTP/HTTPS/TCP 接続を行うシンプルな外形監視であり、プログラムのソースコードを必要とせず（選択肢B）、コンテナイメージも不要です（選択肢D）。コンテナ管理は GKE や Cloud Run の設定で行います。"
    },
    {
      "id": "q126",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "Pod が Pending 状態になる主な原因は、スケジューリング失敗とイメージ pull 失敗の 2 種類です。ノードにリソースが不足している場合（CPU/メモリの requests がノードの空きを超える）、Kubernetes スケジューラーが Pod を配置できず Pending が続きます（選択肢A）。また、コンテナイメージが存在しない・タグが誤っている・認証エラーで pull できない場合も ImagePullBackOff → Pending になります（選択肢B）。Liveness Probe の失敗は Running 状態の Pod を再起動させるもので、Pending の原因ではありません（選択肢C）。Service の ClusterIP は Pod のスケジューリングと無関係であり、Service がなくても Pod は Running になります（選択肢D）。"
    },
    {
      "id": "q130",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "組織ポリシーのカスタム制約（Custom Constraints）を使ってプロジェクト作成リクエストに必須ラベルが含まれていない場合を DENY することで、ラベルのないプロジェクトの作成を API レベルで防止できます。これにより、すべての新規プロジェクトに必ずコストセンタータグが付与されることが保証されます。人手への依存（選択肢A）はヒューマンエラーがあります。Cloud Functions でのイベント検知（選択肢B・C）は事後処理であり、ラベルなしプロジェクトが一時的に存在してしまいます。カスタム制約は作成前に強制できるため最も確実です。"
    },
    {
      "id": "q132",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "Binary Authorization は GKE と Cloud Run（一部のモード）へのコンテナデプロイを、イメージに付与された署名（Attestation）のポリシーに基づいて制御します（選択肢A）。ポリシーで特定の Attestor（例: 脆弱性スキャン合格・CI/CD パイプラインの署名）が証明したイメージのみデプロイを許可することで、未承認イメージの本番デプロイを防止できます（選択肢C）。Binary Authorization は Compute Engine の VM イメージではなく、コンテナイメージを対象としています（選択肢B は誤り）。Binary Authorization は Cloud Armor とは独立したサービスです（選択肢D は誤り）。"
    },
    {
      "id": "q134",
      "type": "single",
    ...
    },
    {
      ...
      "domain": "アクセスとセキュリティの構成",
      "difficulty": "hard",
      "question": "マルチテナントの SaaS アプリケーションで、テナントごとに異なる Cloud KMS キーを使ってデータを暗号化しています。テナント A のデータにテナント B のキーを使った暗号化が行われないよう保証するための最も適切な設計はどれですか？",
      "choices": [
