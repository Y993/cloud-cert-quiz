...
window.CERT_EXAMS["aws-sap-c02"] = {
  ...
  "questions": [
    ...
    },
    {
      ...
      "explanation": "AWS RAM を使った VPC 共有（Shared VPC）は、ネットワークアカウントが VPC サブネットを他アカウントと共有し、各アカウントがそのサブネットにリソースをデプロイできる仕組みです。VPC は1つで済むため管理が集中化でき、運用負荷を最小化できます。\n\nD: VPC ピアリングは推移的ルーティングをサポートしないため、アカウント数が増えると管理が複雑になります。各アカウントに VPC を作成する必要があるため共有サブネットの要件を直接満たしません。\n\nA: Transit Gateway はルーティングハブとして有用ですが、各アカウントに VPC とサブネットを別途作成する必要があり、「共有サブネット」への直接デプロイとはなりません。\n\nC: AWS PrivateLink はサービスエンドポイントの共有に使うもので、サブネットリソースの共有には対応していません。"
    },
    {
      "id": "q003",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "GuardDuty の Organizations 統合では、委任管理者アカウントを指定すること（A）で中央管理が可能になります。委任管理者から「自動有効化」を設定すること（D）で、既存・新規メンバーアカウントの GuardDuty が自動的に有効化され、検出結果も委任管理者アカウントに自動集約されます。この2つの設定だけで両要件を満たせます。\n\nE: 手動での有効化は新アカウント追加時に手作業が発生し、運用負荷が高くなります。\n\nB: SCP による禁止は有効ですが、検出結果の集約には別途設定が必要で、自動有効化の代替にはなりません。\n\nC: Security Hub は GuardDuty の検出結果を集約できますが、GuardDuty 自体の有効化を自動化する機能ではなく、要件⑤を満たしません。"
    },
    {
      "id": "q005",
      "type": "single",
      ...
      "explanation": "AWS IAM Identity Center は Organizations と統合しており、1か所で複数アカウントへの SSO と権限セット管理が可能です。オンプレミス AD とは AD Connector（既存 AD に直接接続）または AWS Managed Microsoft AD（信頼関係設定）でフェデレーションできます。将来の SaaS アプリへの SSO も IAM Identity Center から設定できます。\n\nB: 各アカウントへの IAM ユーザー作成は、アカウント数が増えると管理が指数的に複雑になり、SSO の要件を満たしません。\n\nD: アカウントごとに SAML フェデレーションを設定する方法は可能ですが、80 アカウントなどの大規模環境では設定と管理が非常に煩雑です。IAM Identity Center はこの課題を解決するために設計されています。\n\nC: Cognito は主にエンドユーザー向け Web/モバイルアプリの認証に使うサービスであり、社員の AWS マネジメントコンソールアクセス管理には適していません。"
    },
    {
      "id": "q006",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "GuardDuty の委任管理者設定（A）、Security Hub の委任管理者設定（C）、Config Aggregator の設定（D）の3つがそれぞれ必要です。これらを組み合わせることで、脅威検出・セキュリティ標準準拠・リソース設定コンプライアンスをすべて中央から管理できます。\n\nE: CloudTrail の手動エクスポートは組織レベルのトレイルを使えば自動化できます。手動での各アカウントへの設定は非効率で管理負荷が高く、必須要件ではありません。\n\nB: Amazon Detective は GuardDuty と統合した調査ツールとして有用ですが、今回の要件（集約・ダッシュボード）の「必要な設定」ではなく、オプションの追加機能です。3つの選択肢は A・C・D の設定が核心部分です。"
    },
    {
      "id": "q015",
      "type": "single",
      ...
      "explanation": "Transit Gateway を中心とした構成では、複数の Direct Connect 接続（異なる DX ロケーション）と Site-to-Site VPN を単一の Transit Gateway に集約できます。BGP の AS-PATH プリペンドやコミュニティを使って Direct Connect を優先ルートとし、両 DX 回線が切断された場合に自動で VPN にフェイルオーバーする設計が実現できます。\n\nD: 複数の VGW に分割すると、VPC ごとに異なるルーティングが必要になり管理が複雑化します。Transit Gateway を使うことで複数 VPC への接続も一元管理できます。\n\nA: VPN を1本の VGW にのみ設定すると、もう一方の DX ロケーションへの接続失敗時にフェイルオーバーパスが不完全になります。\n\nB: DX の冗長性だけでは「両回線切断時の VPN フェイルオーバー」要件を満たしません。自然災害や広域障害時に両 DX ロケーションが同時に影響を受けるリスクがあります。"
    },
    {
      "id": "q016",
      "type": "single",
      ...
      "explanation": "IAM Access Analyzer の未使用アクセス分析は、実際に使用されているアクションのみを特定し、最小権限ポリシーの生成を自動化します。これを Organizations レベルで有効化することで全アカウントをスキャンできます。Identity Center に統合して IAM ユーザーを排除することで、認証情報の集中管理と権限の一元管理が実現します。\n\nD: 手動の四半期レビューは規模が大きくなると非現実的で、継続的な最小権限の維持には不十分です。\n\nA: Trusted Advisor は過剰な権限の一部を検出しますが、未使用アクション分析の精度と網羅性は IAM Access Analyzer に劣り、IAM Identity Center への移行を促進しません。\n\nC: 危険なアクションの一括 Deny は迅速な対処には見えますが、例外が増え管理が複雑になります。正確な最小権限ポリシーの作成には実際の使用データに基づくアプローチが必要です。"
    },
    {
      "id": "q017",
      "type": "single",
      ...
      "explanation": "Transit Gateway を使った集中型 Egress 構成（Centralized Egress）は AWS が推奨するマルチアカウントのインターネットアクセス管理パターンです。NAT Gateway を1か所（Networking アカウント）に集約することで、IP アドレス管理、コスト削減、トラフィック制御が一元化されます。\n\nC: 各 VPC に NAT Gateway を個別作成すると、コストが増加し、管理が分散してセキュリティポリシーの適用も複雑になります。\n\nB: PrivateLink は AWS サービスやプライベートサービスへの接続を提供するものであり、インターネットへのアクセスを提供する機能はありません。\n\nA: VPC ピアリングは推移的ルーティングをサポートしないため、スポーク VPC から Networking VPC を経由してさらにインターネットへ出るルーティングは標準的な VPC ピアリングでは実現できません。"
    },
    {
      "id": "q018",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "AD Connector は AWS がオンプレミス AD への認証リクエストをプロキシする軽量なディレクトリゲートウェイです。ユーザーやグループのコピーや同期は不要で、EC2 インスタンスは既存の AD ドメインに直接参加できます。AD の管理もオンプレミスのチームが継続できます。\n\nD: Managed Microsoft AD との双方向フォレストトラストは機能しますが、AWS 側に新しい AD を作成・管理するコストと負荷が生じます。「AD 管理はオンプレミスのチームが継続」という要件に対してオーバーエンジニアリングです。\n\nA: ユーザーとグループを手動移行すると、オンプレミス AD との二重管理が発生し、同期の問題が生じます。要件に反します。\n\nB: Cognito は Web/モバイルアプリのエンドユーザー向け認証サービスであり、EC2 インスタンスのドメイン参加や AD グループベースのアクセス管理には対応していません。"
    },
    {
      "id": "q023",
      "type": "single",
      ...
      "explanation": "組織レベルの CloudTrail により全本番アカウントのログを中央集約できます。S3 Object Lock の COMPLIANCE モードは保持期間中のオブジェクト削除・変更をルートユーザーを含むすべてのユーザーから防止し、改ざん防止に最適です。CloudTrail ログファイル整合性検証により改ざん検出が可能です。KMS CMK 暗号化でデータの機密性を確保し、バケットポリシーでセキュリティチームのみのアクセスを実現します。\n\nC: 各アカウントでの個別設定は管理が分散し、S3 Lifecycle は削除を制御しますが WORM（Write Once Read Many）による改ざん防止は提供しません。\n\nB: CloudWatch Logs は CloudTrail の長期保存に設計されておらず、7 年間の WORM 保存には Object Lock を持つ S3 が適切です。また整合性検証機能も CloudWatch Logs には組み込まれていません。\n\nA: AWS Backup は EC2/EBS/RDS などのバックアップが主目的であり、CloudTrail ログの WORM 保護には対応していません。S3 Object Lock と CloudTrail 整合性検証の組み合わせが金融規制要件には最適です。"
    },
    {
      "id": "q024",
      "type": "single",
      ...
      "explanation": "API Gateway + Lambda のサーバーレス構成はゼロから無制限のスケールに自動対応し、深夜のアイドル時はほぼコストがかかりません。Aurora Serverless v2 は需要に応じてキャパシティを自動スケールし、リレーショナルモデルと複雑な JOIN をサポートします。ElastiCache Redis で読み取り 80% をキャッシュすることで P99 100ms の要件を達成できます。\n\nD: EC2 Auto Scaling + RDS Multi-AZ は機能しますが、深夜のアイドル時もインスタンスコストが発生し、コスト効率が低下します。インフラ管理の運用負荷も Lambda より高くなります。\n\nA: DynamoDB はリレーショナルモデルと複雑な JOIN クエリには不向きです。JOIN が必要なデータモデルには RDS/Aurora が適しています。\n\nC: スポットインスタンスはコスト削減に有効ですが、中断リスクがあり、P99 レイテンシー 100ms の要件への影響や運用の複雑性が増します。深夜のトラフィックゼロ時のコスト最適化もサーバーレスほど効果的ではありません。"
    },
    {
      "id": "q025",
      "type": "single",
      ...
      "explanation": "AWS Elemental MediaConvert はマネージドの動画トランスコードサービスで、インフラ管理不要です。S3 イベントで自動的にジョブを開始でき、複数解像度への同時変換、30 分の処理時間、2 時間以内の完了要件を満たせます。CloudFront との統合で配信も最適化されます。Reserved Transcode により日次処理量が多い場合はコスト削減できます。\n\nB: 24 時間稼働の EC2 フリートは処理がない時間もコストがかかり、コスト最小化に反します。トランスコード処理を自前で実装・運用する手間も生じます。\n\nD: AWS Batch + Spot も有効ですが、Spot インスタンスの中断が発生すると 2 時間の SLA を超えるリスクがあります。また、トランスコード処理を自前で用意する必要があります。\n\nA: Lambda の最大実行時間は 15 分であり、平均 30 分のトランスコードには対応できません。"
    },
    {
      "id": "q026",
      "type": "single",
      ...
      "explanation": "Kinesis Data Streams は毎秒数万件のストリームデータ取り込みに最適化されています。Kinesis Data Analytics（Flink）でリアルタイムのウィンドウ処理と前処理を行い、SageMaker リアルタイム推論で P99 100ms のスコアリングが実現できます。Kinesis Data Firehose で S3 への自動保存と 90 日保持が実現でき、SNS でアラートを即時送信できます。\n\nD: SQS + Lambda は高スループットのリアルタイムストリーミングよりもキューベースの非同期処理に向いており、毎秒 50,000 件の処理と 100ms レイテンシーの達成が困難です。Lambda のコールドスタートもレイテンシーに影響します。\n\nB: IoT Core は IoT デバイスからのデータ収集を主目的としており、金融取引データの高スループット処理には Kinesis が適しています。\n\nC: MSK + ECS Fargate + Aurora の構成は機能しますが、インフラの管理負荷が高く、100ms のレイテンシー要件の達成にはチューニングが必要です。Kinesis のフルマネージドな解決策より運用負荷が増します。"
    },
    {
      "id": "q027",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "AWS IoT Core は数十億のデバイス接続と毎秒数兆件のメッセージを処理できるマネージドサービスです。IoT ルールエンジンで Kinesis に転送し、Kinesis Analytics で低レイテンシーの異常検知、Kinesis Firehose で S3 への自動保存が実現できます。IoT Core のデバイスシャドウで双方向通信（コマンド送信）も提供されます。\n\nB: SQS + Lambda は高スループットの IoT ワークロードには適しておらず、毎秒 100 万件のメッセージ処理には Lambda の並列実行コストが膨大になります。RDS PostgreSQL は書き込みスケールに限界があります。\n\nA: Amazon MQ は中規模の既存 MQTT ワークロードの移行に適しており、100 万台規模のデバイス接続には設計されていません。スケーラビリティに限界があり、マネージドの IoT Core に比べて運用負荷が高くなります。\n\nC: ALB の WebSocket を IoT デバイスの接続管理に使うことは、IoT ワークロードの特性（デバイス認証、MQTT プロトコル、デバイスシャドウ等）を無視しており、100 万台規模への対応も困難です。"
    },
    {
      "id": "q032",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "AWS Glue（D）はサーバーレスの ETL サービスで、インフラ管理なしでデータのクレンジング・変換ができます。Glue の Spark エンジンは大規模データ処理に対応しており、Redshift へのロードも Glue Connector で実現できます。Step Functions（C）はサーバーレスのワークフローオーケストレーターで、各ステップの成功/失敗の追跡、失敗時の自動再試行（Retry）、エラーハンドリング（Catch）を設定ファイルで定義できます。\n\nE: Amazon EMR はサーバーレスではなく、クラスターの管理が必要です（EMR Serverless を使えば変わりますが、選択肢には EMR クラスターと記載されています）。\n\nA: EC2 Auto Scaling は完全にインフラ管理を排除できないため、「インフラ管理を完全に排除」という要件に反します。\n\nB: AWS Data Pipeline はレガシーサービスで、Step Functions に比べて機能が限定的です。複雑なワークフロー管理には Step Functions が適しています。"
    },
    {
      "id": "q034",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "DynamoDB Global Tables はマルチリージョンのアクティブ-アクティブ書き込みをサポートし（要件③）、TransactWriteItems API で ACID トランザクション（要件④）を提供します。また、グローバルテーブルは自動的にリージョン間でデータを同期し（通常1秒未満のレプリケーション遅延）、リージョン障害時の自動フェイルオーバーも提供されます（要件②①）。\n\nD: RDS Multi-AZ + クロスリージョンリードレプリカは読み取りのみリードレプリカへの転送ができますが、マルチリージョンへの書き込みは単一の書き込みエンドポイントからのみで、要件③を満たしません。\n\nC: Aurora Global Database は通常、1 リージョンのプライマリにのみ書き込み可能で、マルチリージョン書き込み（アクティブ-アクティブ書き込み）には対応していません（ライト-ワン構成）。選択肢も最終的に DynamoDB への移行を提案しており、Aurora Global Database 単独では要件を満たせません。\n\nA: DocumentDB は MongoDB API 互換のドキュメントデータベースで、グローバルテーブルのようなマルチリージョンのアクティブ-アクティブ書き込みはサポートしていません。"
    },
    {
      "id": "q037",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "RDS Proxy は Lambda と RDS の間でコネクションプーリングを管理し、Lambda の同時実行が 1,000 であっても RDS への実際の接続数を数十に削減できます。Lambda 側はコネクション文字列を RDS エンドポイントから RDS Proxy エンドポイントに変更するだけで対応でき、コードの変更は最小限です。\n\nC: インスタンスのアップグレードでmax_connections は増えますが、Lambda の同時実行が増加するにつれ再度上限に達します。根本的な解決にはなりません。\n\nA: 同時実行数を 50 に制限すると、ピーク時に多数のリクエストがキューイングまたはエラーになり、アプリケーションのパフォーマンスが大幅に低下します。\n\nB: DynamoDB への移行は PostgreSQL が必要なリレーショナルモデルの場合に適用できず、大規模なデータモデルの変更とアプリケーションの書き直しが必要です。「コードを最小限に変更」という要件に反します。"
    },
    {
      "id": "q040",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "S3 Lifecycle ポリシーは既知のアクセスパターン（30 日→IA、90 日→Glacier）への自動移行に対応します。S3 Intelligent-Tiering はアクセスパターンが不明なデータを監視し、アクセス頻度に応じて自動でストレージティアを移動し、追加の Lifecycle ルール設定なしにコスト最適化を実現します。両方を組み合わせることで最大のコスト削減が期待できます。\n\nD: Glacier Deep Archive は最も低コストですが、データアクセスに最大 48 時間かかり、30 日以内に頻繁にアクセスされるデータには不適切です。\n\nA: バージョニングの有効化と古いバージョンの移行は、コスト削減の一部にはなりますが、現在のオブジェクト（最新バージョン）の最適化には直接対応していません。\n\nC: S3 から EFS への移行はコスト面で逆効果です。EFS は EC2 などのファイルシステムアクセスが必要な場合に適しており、大容量のオブジェクトストレージの代替にはなりません。EFS のコストは S3 より高くなります。"
    },
    {
      "id": "q046",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "X-Ray は分散トレーシングを提供し、マイクロサービス間のリクエストフローを可視化します。X-Ray デーモンの DaemonSet デプロイにより、コードの最小限の変更でトレース収集が可能です。CloudWatch Container Insights はコンテナとポッドレベルのメトリクスを提供し、ADOT でのメトリクスとトレースの標準化で OpenTelemetry エコシステムとの互換性も確保できます。\n\nC: 手動での相関 ID 伝播は開発負荷が高く、すべてのサービスに実装するには時間がかかります。標準化されたトレーシングフレームワークを使う方がはるかに効率的です。\n\nB: kubectl top は基本的なリソース確認に有効ですが、分散トレーシングやアプリケーションレベルのボトルネック特定には不十分です。手動での再起動は根本原因の解決になりません。\n\nA: S3 + Athena でのログ分析は事後分析に有効ですが、リアルタイムの障害診断や分散トレーシングには対応しておらず、原因特定の時間短縮には効果が限定的です。"
    },
    {
      "id": "q049",
      "type": "multiple",
      ...
      "explanation": "Systems Manager Patch Manager（B）はパッチベースラインと Maintenance Window の組み合わせで EC2 への自動パッチ適用を実現し、問題②を継続的に解決します。AWS Config のリメディエーション（E）は非準拠の S3 バケットを自動検出して Lambda で修復することで問題⑤を継続的に解決します。この2つが「自動化された継続的な修正」の要件に最も合致します。\n\nD: Trusted Advisor の週次メール送信は手動対応を促すもので、「自動修正」ではありません。\n\nA: SCP で MFA 強制は有効ですが、これは「既存の IAM ユーザーに MFA を設定する」修正ではなく、MFA なしのアクセスを拒否する予防的制御です。既存ユーザーへの MFA 設定は自動化できません。\n\nC: Amazon Inspector は脆弱性の検出に有効ですが、パッチ適用の自動化は Inspector 単独では実現できず、Systems Manager との連携が別途必要です。"
    },
    {
      "id": "q050",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "WAF ロギングの有効化により、ブロックされた/許可されたリクエストの詳細（IP、URI、マッチしたルール等）を完全に把握できます。Athena + QuickSight でのログ分析でパターンの可視化が可能です。新ルールのカウントモードテストは本番トラフィックに影響なく誤検知率を測定し、安全確認後にブロックモードに切り替えることで段階的な展開が実現できます。\n\nC: WAF ルールの一時無効化は本番セキュリティの空白期間を作り、攻撃に対して無防備になります。\n\nD: Shield Advanced はDDoS 保護の強化に有効ですが、WAF の誤検知問題や可視性の改善には直接対応しません。WAF ロギングは Shield Advanced なしでも設定できます。\n\nB: WAF ルールの全削除は論外であり、アプリケーションを攻撃に対して完全に無防備にします。"
    },
    {
      "id": "q052",
      "type": "single",
      ...
      "explanation": "キャッシュヒット率の低下の主な原因は、不必要なクエリパラメーターや Cookie がキャッシュキーに含まれていることです。これにより同じコンテンツが異なるキャッシュエントリとして扱われます。不要なキャッシュキーを除外し、Cache-Control ヘッダーで適切な TTL を設定することでヒット率が大幅に向上します。Origin Shield はキャッシュの中間層として追加のキャッシュ効果を提供します。\n\nC: CloudFront を削除することはパフォーマンスをさらに悪化させ、CDN の恩恵を失います。\n\nD: Lambda@Edge でリクエストをオリジンに転送することはキャッシュヒット率の改善に逆効果で、処理コストも増加します。\n\nA: ElastiCache はアプリケーション層のキャッシュを改善しますが、CloudFront のキャッシュヒット率（エッジでの命中）の問題には直接対応しません。CloudFront の設定変更なしでは改善が限定的です。"
    },
    {
      "id": "q053",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "RDS Proxy は Aurora のフェイルオーバーをプロキシ層で処理します。フェイルオーバー時にアプリケーションは RDS Proxy エンドポイントへの接続を維持したまま、Proxy が内部的に新しいプライマリへの接続を確立します。アプリケーション側の変更は接続文字列をクラスターエンドポイントから RDS Proxy エンドポイントに変更するだけで、フェイルオーバー時間を数秒程度に短縮できます。\n\nC: フェイルオーバー優先度の設定はどのレプリカが新プライマリになるかに影響しますが、フェイルオーバー後のアプリケーション接続時間の問題を解決しません。\n\nD: Aurora クラスターエンドポイントは既にフェイルオーバー後の新プライマリを指すよう DNS が更新されますが、DNS の TTL（通常 5 秒）と接続プールの古い接続が問題です。Route 53 ヘルスチェックを追加しても根本的な接続プールの問題は残ります。\n\nA: アプリケーションへの再試行ロジック追加は有効ですが、既存ドライバーのフェイルオーバーサポートがない場合は実装が複雑になります。「最小限のアプリケーション変更」という要件に対して RDS Proxy の方が少ない変更で対応できます。"
    },
    {
      "id": "q056",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "Systems Manager Patch Manager（C）のコンプライアンスレポートは、全インスタンスのパッチ適用状況（準拠/非準拠）を一元的なダッシュボードで確認でき、要件③を満たします。Systems Manager Inventory（D）はエージェント経由でインストール済みアプリケーション、サービス、ネットワーク設定等を自動収集し、要件④を満たします。両方とも SSM Agent がインストールされている EC2 インスタンスに対して最小限の設定で有効化できます。\n\nE: cron ジョブによる手動収集は自動化されておらず、管理負荷が高く、一元管理もできません。\n\nB: Amazon Inspector は CVE の脆弱性スキャンに特化しており、パッチコンプライアンスの一元ダッシュボードとしての機能や、インベントリ収集は Patch Manager と Inventory の方が直接対応しています。\n\nA: Trusted Advisor のセキュリティチェックは一部のセキュリティ設定を確認しますが、EC2 のパッチコンプライアンス詳細レポートやインベントリ収集は提供していません。"
    },
    {
      "id": "q059",
      "type": "single",
      ...
      "explanation": "API Gateway の使用量プランは特定の API キー（ユーザーまたはグループに発行）に対して、スロットリング（秒あたりのリクエスト数）とクォータ（月あたりの総リクエスト数）を細かく設定できます。アカウントレベルとステージレベルのデフォルトスロットリングと組み合わせることで、多層的なレート制限が実現できます。設定は AWS マネジメントコンソールから行えます。\n\nD: Lambda を使ったカスタムレート制限の実装は高い開発コストがかかり、DynamoDB への書き込みレイテンシーがリクエスト処理に影響します。API Gateway のネイティブ機能を使う方がはるかに効率的です。\n\nA: CloudFront はキャッシュによるオリジンへのリクエスト削減に有効ですが、ユーザーグループごとの細かいレート制限設定は API Gateway の使用量プランが適しています。\n\nC: 同時実行数の増加は過剰なリクエストをさばく容量を増やしますが、過剰なリクエストそのものを制限するレート制限の要件を解決しておらず、コストも増大します。"
    },
    {
      "id": "q060",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "AWS MGN はブロックレベルの継続的レプリケーション（CDC）でオンプレミス VM のデータを AWS に継続的に同期します。カットオーバー時は最後の差分のみ同期するため数分〜30 分でカットオーバーが完了します。アプリケーションへの変更は一切不要です（リフト＆シフト）。ソースサーバーを保持したままにすることで、ロールバックが必要な場合はオンプレミスの VM を再開できます。\n\nD: SMS は MGN の前世代のサービスで、現在は MGN の使用が推奨されています。SMS はサーバー単位のイメージ（AMI）ベースのレプリケーションで、継続的なブロックレベル同期ではありません。\n\nA: VM Import/Export は一時点のスナップショットをインポートするもので、継続的な差分同期ができず、30 分以内のカットオーバーの保証が困難です。\n\nC: インフラの再構築とアプリケーションの再デプロイは「アプリケーションへの変更は一切不可」という要件に反します。"
    },
    {
      "id": "q067",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "フェーズ1: EC2 + RDS MySQL は最もシンプルなリフト＆シフトで、既存のアーキテクチャをほぼそのまま移行できます。フェーズ2: ECS（Fargate）でコンテナ化し、アプリを変更せずにコンテナで実行します。フェーズ3: マイクロサービス化では ECS/EKS で各サービスを独立したコンテナに分割し、Aurora でパフォーマンスを向上させ、API Gateway でサービス間通信を管理、EventBridge でイベント駆動の非同期連携を実現します。\n\nA: フェーズ1 で Lambda + DynamoDB に移行することは、Spring Boot の全書き直しが必要で「リフト＆シフト」ではありません。最初のフェーズとして不適切です。\n\nD: Elastic Beanstalk はアプリデプロイの管理を簡素化しますが、フェーズ3 のマイクロサービス化には ECS/EKS の方が適しており、Elastic Beanstalk からの移行が改めて必要になります。\n\nB: フェーズ2 で Lambda + DynamoDB への移行は Spring Boot + MySQL の全書き直しが必要で、段階的なアプローチの流れに合いません。"
    },
    {
      "id": "q073",
      "type": "multiple",
      ...
      "explanation": "SCT（D）はスキーマ変換ツールとして、Oracle/SQL Server などからターゲット（Aurora/MySQL/PostgreSQL）への変換の複雑度を「自動変換可能」「手動対応必要」に分類してレポートします。移行工数の見積もりに直接役立ちます。DMS フリートアドバイザー（E）は複数データベースに接続して利用状況（サイズ、接続数、活動度）をスキャンし、どのエンジンに移行すべきか、TCO の比較を提供します。この2つが前評価フェーズの自動化に最適です。\n\nA: 手動調査は正確性が低く、大規模なデータベース群の前評価には非現実的です。\n\nB: Trusted Advisor はデータベースの利用状況（Idle RDS インスタンスなど）の確認には有用ですが、スキーマ複雑度の評価や移行戦略のレコメンデーションは提供しません。\n\nC: Detective はセキュリティ調査ツールであり、データベース移行の前評価には関係ありません。"
    },
    {
      "id": "q074",
      "type": "single",
      ...
      "choices": [
        "GitHub などのソースリポジトリでコードを管理し、コードプッシュをトリガーに CodePipeline が起動する。CodeBuild でユニットテスト・セキュリティスキャン・Docker イメージのビルドを自動実行し、CodeDeploy で Blue/Green デプロイを実施する。インフラは CloudFormation または Terraform でコード管理し、各環境のパラメーターを環境変数とパラメーターストアで管理する",
      ...
      "explanation": "AWS の CI/CD スタック（GitHub などのソース + CodePipeline + CodeBuild + CodeDeploy）はパイプラインとして統合でき、ソースコードのプッシュから本番デプロイまでを自動化できます。CodePipeline は GitHub や S3 をソースプロバイダとして利用でき、CodeBuild でテストとビルドを自動化し、CodeDeploy の Blue/Green でリリースのリスクを最小化します。CloudFormation/Terraform でインフラをコードとして管理し、Systems Manager パラメーターストアで環境ごとの設定を管理することで、環境間の差異問題を解決します。\n\nD: Jenkins の EC2 インストールはサーバーの管理負荷が生じ、手動デプロイスクリプトは「完全自動化」の要件を満たしません。\n\nC: GitHub Actions は CI として有効ですが、本番デプロイに手動承認と AWS CLI を使う方法は完全自動化ではなく、環境設定のコード管理も部分的な対応です。\n\nB: Elastic Beanstalk は有効ですが、複雑なマルチ環境の CI/CD パイプラインや高度なデプロイ戦略（Blue/Green）の管理には CodePipeline の方が柔軟性が高いです。"
    },
    {
      "id": "q075",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "Amazon FSx for Windows File Server は Windows ネイティブの SMB プロトコル、NTFS、Active Directory との統合をネイティブサポートするマネージドサービスです。既存の Windows ファイルサーバーとほぼ同じ操作性で使用でき、AD ベースのアクセス制御もそのまま維持できます。DataSync による移行後、DNS の切り替えだけでユーザーには透過的に移行できます。\n\nC: S3 + S3 File Gateway は有効ですが、SMB ゲートウェイの管理が必要で、ネイティブの FSx for Windows File Server より機能が限定的です（特に AD 統合や Windows ACL の完全サポート）。\n\nD: Amazon EFS は Linux/NFS プロトコル向けのマネージドファイルシステムであり、SMB プロトコルはサポートしていません。\n\nA: S3 は オブジェクトストレージであり、SMB プロトコルでの直接アクセスには対応していません。ユーザーが同じようにアクセスできるという要件を満たしません。"
    },
    {
      "id": "q077",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "大規模移行の包括的なビジネスケース作成には複数の要素が必要です。Migration Evaluator でオンプレミスの実際のデータに基づく正確な TCO を算出し、AWS CAF でビジネス・人員・ガバナンス・技術の 6 つの視点からのギャップと準備状況を評価します。AWS Professional Services や認定パートナーの専門知識を活用することで、経営陣や規制当局が求める品質のビジネスケースドキュメントを作成できます。\n\nC: Pricing Calculator と内部チームによる手動作成は精度と網羅性に限界があり、コンプライアンス要件を満たす詳細なビジネスケースの作成が困難です。\n\nA: Cost Explorer は既存の AWS コストを分析するツールであり、オンプレミスシステムの移行ビジネスケースの作成には対応していません。\n\nB: Excel による手動計算は時間がかかり、100 台のサーバーに対して正確なデータを収集・分析することは困難で、ビジネスケースの品質と信頼性が低くなります。"
    },
    {
      "id": "q081",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "Amazon Cognito User Pool はユーザー認証と認可に必要なすべての機能をマネージドサービスとして提供します。メール/パスワード認証、ソーシャルログイン（OIDC/SAML フェデレーション）、MFA（SMS/TOTP）、JWT トークン発行（ID/Access/Refresh）をすべて組み込みでサポートし、API Gateway の Cognito オーソライザーと直接統合できます。\n\nB: IAM ユーザーはアプリケーションユーザーの管理には設計されておらず、ソーシャルログインや MFA の統合が複雑になります。\n\nD: AWS Directory Service は企業の AD/LDAP 環境との統合に適しており、コンシューマー向けアプリのユーザー認証要件（ソーシャルログイン等）には設計されていません。\n\nA: サードパーティ IdP の使用は機能的ですが、「最小限の開発負荷で AWS サービスを使う」要件に対して、追加のベンダー管理コストと統合の複雑さが生じます。"
    },
    {
      "id": "q084",
      "type": "multiple",
      ...
      "explanation": "DynamoDB Global Tables（B）は両リージョンでの書き込みを可能にするアクティブ-アクティブデータベース構成を提供します。リージョン間のデータレプリケーションは自動で行われ、ACID トランザクションをサポートします。Route 53 ヘルスチェック + レイテンシーベースルーティング（E）により、ユーザーは通常時は最も近いリージョンにルーティングされ、障害時はヘルスチェック失敗のリージョンを自動的に除外して正常なリージョンにフェイルオーバーします。\n\nA: RDS のクロスリージョンリードレプリカは読み取り専用であり、eu-west-1 での書き込みができないため、アクティブ-アクティブ構成には対応していません。\n\nD: CloudFront は CDN として静的コンテンツの配信に有用ですが、アクティブ-アクティブの設計要素（データの双方向書き込み、自動フェイルオーバー）には対応していません。\n\nC: S3 の双方向レプリケーションはオブジェクトストレージには有効ですが、アプリケーションデータのアクティブ-アクティブ書き込みの主要コンポーネントではありません。"
    },
    {
      "id": "q085",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "EventBridge はサーバーレスのイベントバスで、イベントパターンマッチング（source、detail-type、任意の detail フィールド）に基づく細かなルーティングが可能です。新しいルールを追加するだけで既存コードを変更せずに新しいイベントタイプやコンシューマーを追加できます。Schema Registry でスキーマを管理し、イベント駆動アーキテクチャのドキュメントとして活用できます。\n\nB: Kinesis + Lambda でのカスタムルーティングは、新しいイベントタイプ追加のたびにコード変更が必要で、拡張性の要件に反します。\n\nA: SNS は有効ですが、複数の属性を組み合わせた複雑なフィルタリング（メッセージフィルタリングポリシー）は EventBridge のイベントパターンマッチングほど柔軟ではありません。\n\nC: SQS FIFO は順序付き処理に有効ですが、イベントのルーティング（異なるコンシューマーへの振り分け）機能がなく、単一のコンシューマーへの配信のみです。"
    },
    {
      "id": "q090",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "テナントごとに CMK を作成することで、暗号化の分離と暗号学的消去が実現できます。ScheduleKeyDeletion でキーを削除すると、そのキーで暗号化されたすべてのデータ（S3、RDS、DynamoDB など）は物理的に削除しなくても永続的に復号不可能になります。これが「暗号学的消去」の本質です。テナント間の鍵の分離も保証されます。\n\nC: 単一のマネージドキーでの暗号化はテナント間の暗号化分離がなく、暗号学的消去も実現できません。すべてのテナントデータが同じキーで暗号化されているため、特定テナントのデータのみを消去できません。\n\nA: S3 Object Lock は WORM（Write Once Read Many）で削除防止に使うものであり、暗号学的消去とは逆の目的です。解約時のデータ消去要件に反します。\n\nB: テナントごとのアカウント分離は強力ですが、コスト的に非現実的で、アカウント削除には AWS の正式なプロセスが必要です。また「暗号学的消去」ではなく物理的削除になります。"
    },
    {
      "id": "q096",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "EC2 スポットインスタンスはオンデマンド比最大 90% のコスト削減が可能で、チェックポイントがある中断耐性のバッチジョブには最適な選択です。Spot Instance Interruption Notice の 2 分前通知をインスタンスメタデータから取得し、状態をチェックポイントとして保存することで、中断後の再開が可能です。複数のインスタンスタイプとアベイラビリティゾーンを指定することで、Spot の可用性を高めることができます。\n\nC: Reserved Instances の購入はバッチジョブが断続的に実行される場合（24 時間365日でない）に非効率です。使わない時間も料金が発生します。\n\nB: Lambda の最大実行時間は 15 分で、2〜8 時間のバッチジョブには直接対応できません。処理の分割と再設計には大きな開発コストが必要です。\n\nA: Savings Plans はコンピューティングコストを削減しますが、スポットインスタンスほどの大幅な削減（最大 90%）は達成できません。Savings Plans での最大削減は約 66% です。"
    },
    {
      "id": "q099",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "AWS Network Firewall（B）はステートフルな深いパケット検査（DPI）、侵入防止システム（IPS）、FQDN ベースのフィルタリング（ドメイン名での許可/拒否リスト）を提供します。集中型 Egress VPC に配置することで、全アカウントのアウトバウンドトラフィックを一元的に検査できます。Transit Gateway（C）は全アカウントの VPC を集中型 Egress VPC に接続するルーティングハブとして機能し、アウトバウンドトラフィックを Network Firewall 経由に強制するルートテーブル設計が実現できます。\n\nE: セキュリティグループの IP ホワイトリストは FQDN ベースのフィルタリングに対応しておらず、ドメイン名での制御は直接できません。また各アカウントへの個別設定は管理が分散します。\n\nD: GuardDuty は脅威検出と通知が目的であり、リアルタイムのトラフィックブロック（防止）機能はありません。\n\nA: WAF は Web アプリケーション（HTTP/HTTPS）への受信トラフィックの制御に特化しており、アウトバウンドの FQDN フィルタリングには対応していません。"
    },
    {
      "id": "q101",
      "type": "single",
      ...
      "explanation": "DynamoDB Accelerator（DAX）はインメモリキャッシュクラスターで、繰り返しの読み取りをマイクロ秒レベルでキャッシュから応答します。ホットパーティションへの毎秒 1 万件の読み取りが同一データの繰り返しであれば、DAX のキャッシュヒット率は非常に高くなり、DynamoDB への読み取りを大幅に削減できます。DAX は DynamoDB の API と完全互換で、アプリケーションのコード変更も最小限です。\n\nC: プロビジョニングキャパシティの増加は問題を解決しますが、ホットパーティションの制限（1 パーティションあたりの上限: 1,000 RCU）により、キャパシティを増やしてもパーティションの上限に達する可能性があります。またコストも増加します。\n\nD: パーティションキーのシャーディングは書き込み分散には有効ですが、読み取り時に複数のパーティションを結合する処理が必要になり、アプリケーションの大幅な変更が必要です。\n\nA: DynamoDB から Aurora への移行はデータモデルの全面的な変更が必要で、ホットパーティションという具体的な問題に対して過剰な対応です。"
    },
    {
      "id": "q102",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "AWS Global Accelerator は AWS のグローバルネットワークを使って世界中のエンドユーザーを最も近い AWS エッジロケーション（エニーキャスト IP）に接続し、バックボーンで各リージョンのエンドポイントに転送します。Route 53 の DNS 解決より低いレイテンシーでグローバル接続が改善されます。読み取りは各リージョンの Aurora リードレプリカ、書き込みはプライマリリージョンに分離することでレプリケーション遅延を考慮した設計になります。Shield Advanced + WAF で DDoS 対策が完結します。\n\nD: 各リージョンに独立した書き込み可能な Aurora クラスターを作成するとデータ整合性の問題が生じます。Aurora Global Database のプライマリへの書き込みは必須です。\n\nB: CloudFront は静的コンテンツとキャッシュ可能な API に最適ですが、API キャッシュができない動的 API の全レスポンスをオリジンに転送する場合、Global Accelerator の方がレイテンシーが低くなります。\n\nA: Route 53 のジオロケーションルーティングは DNS ベースのルーティングで切り替えに時間がかかり、Global Accelerator のリアルタイムな障害検知と切り替えには劣ります。また DDoS 対策の統合も別途必要です。"
    },
    {
      "id": "q104",
      "type": "single",
      ...
      "explanation": "専用の「ユーザーサービス」アカウントにデータの単一ソースを確立し、PrivateLink でプライベートかつセキュアな API アクセスを提供するアーキテクチャが最適です。各サービスはユーザーデータを持たず、ユーザーサービスの API を呼び出すだけです。PrivateLink はアカウント間のネットワーク設定を最小化し、サービスのネットワーク詳細を公開せずに API アクセスを提供できます。\n\nB: 全サービスを単一アカウントに統合することは、独立したチームと製品ラインの運営に支障をきたし、セキュリティと可用性の分離も失われます。\n\nA: DynamoDB グローバルテーブルのアカウント間複製は標準的な機能では対応していません（グローバルテーブルはリージョン間のみ）。また双方向同期は競合状態の管理が複雑になります。\n\nC: Cognito のカスタム属性はユーザー認証情報に付随するデータに適していますが、複雑なサブスクリプション状態（複数サービスの組み合わせ、履歴、請求情報等）の管理には機能が限定的です。"
    },
    {
      "id": "q105",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "AWS FIS（D）はカオスエンジニアリング実験を設定ファイルで定義し、EventBridge でスケジュール実行できます。EC2 のシャットダウン、Aurora のフェイルオーバー強制などのインジェクションを自動化し、システムの回復力（Resilience）を定期的に検証します。CloudWatch Synthetics（A）はエンドユーザーのアクション（Headless Chrome ベースの実際のブラウザ操作）をシミュレートし、エンドツーエンドのユーザー体験を継続的に監視します。SRE の観点での④と①の要件に完全に対応しています。\n\nC: AWS Backup による手動リカバリテストは自動化されていなく、四半期に一度では頻度も不十分です。FIS の自動化されたリカバリテストの方がSRE の要件に合います。\n\nB: CloudWatch アラームはインフラメトリクスの監視に有用ですが、エンドツーエンドのユーザー体験の監視はできません。\n\nE: Config は設定管理と監査に適していますが、信頼性テストや実際のユーザー体験監視には対応していません。"
    },
    {
      "id": "q108",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "ACM は DNS 検証を使った証明書の自動発行と自動更新を提供します。CloudFront の Alternate Domain Names に各テナントのドメインを追加し、ACM の証明書を関連付けることで、テナントごとのカスタム HTTPS ドメインが実現できます。テナントが増えても API 経由で自動的にドメインと証明書を追加できます。ACM の証明書は無料で 90 日ごとに自動更新されます。\n\nC: ワイルドカード証明書はサブドメイン（*.example.com）には有効ですが、顧客の独自ドメイン（tenant-a.customdomain.com）には使用できません。\n\nB: テナントごとの EC2 インスタンスは管理が煩雑でコストが高く、テナント数に比例してインフラが増大します。スケーラビリティに欠けます。\n\nA: AWS Certificate Manager Private CA のプライベート証明書は内部通信用であり、パブリックの HTTPS 接続（インターネットからのアクセス）には使用できません。ブラウザは信頼できないプライベート CA の証明書に警告を表示します。"
    },
    {
      "id": "q110",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "Redis の DECR/DECRBY は原子的なオペレーションで、Lua スクリプトを使うことで「現在値 > 0 の場合のみデクリメント」という条件付き原子操作を実現できます。Redis はシングルスレッドで Lua スクリプトを実行するため、完全な原子性が保証されます。ElastiCache Redis はマイクロ秒レベルの応答性能を持ち、高スループットの在庫管理に最適です。\n\nD: PostgreSQL の SELECT FOR UPDATE は有効ですが、高並列の分散マイクロサービス環境では RDS の接続数上限とロック待機が性能ボトルネックになります。\n\nC: DynamoDB の条件付き書き込みは有効なアプローチです。ただし、高い競合率では多数の ConditionalCheckFailedException が発生し、リトライ処理が必要になります。ElastiCache Redis の Lua スクリプトは競合時のパフォーマンスが高く、在庫カウンターのユースケースに特に適しています。\n\nA: SQS FIFO + シリアル処理は確実な順序保証がありますが、高スループット要件ではボトルネックになります。"
    },
    {
      "id": "q112",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "EC2 Windows Server への直接デプロイが最もシンプルなリフト＆シフト方法です。AWS は Windows Server の AMI を提供しており、.NET Framework 4.8 をインストールして既存のアプリケーションをそのままデプロイできます。コードの変更が最小限で、既存の Windows 管理スキルを活用できます。\n\nC: Linux への移植は .NET Framework から .NET Core への書き直しが必要で、「最小限の変更」要件に反します。\n\nA: .NET Core への書き直しは大規模なアプリケーション変更であり、「最小限の変更」要件に大きく反します。\n\nB: Elastic Beanstalk は有効ですが、Windows Server プラットフォームは利用可能なものの、EC2 への直接デプロイよりも Elastic Beanstalk の概念を学ぶ必要があります。最もシンプルという観点では EC2 直接デプロイが最も分かりやすいです。"
    },
    {
      "id": "q118",
      "type": "single",
      ...
      "explanation": "RDS IAM 認証は IAM ロールベースの認証により、パスワードなしでデータベースに接続できます。IAM トークンの有効期限は 15 分で、接続のたびに IAM による認証が必要なため「継続的な検証」のゼロトラスト原則を満たします。EC2 の IAM ロールが侵害されても、IAM ポリシーの変更で即座にアクセスを取り消せます。\n\nC: セキュリティグループは IP ベースのネットワーク制御であり、ゼロトラストの「アイデンティティベースの認証」ではありません。IP は偽装される可能性があります。\n\nB: Secrets Manager からのパスワード取得は認証情報管理を改善しますが、IAM 認証トークンによる継続的な検証（接続ごとの認証）ではなく、パスワード型認証が残ります。\n\nA: 同一サブネットへの配置はネットワーク分離を弱め、セキュリティを向上させません。ゼロトラストはネットワーク境界ではなくアイデンティティ認証を重視します。"
    },
    {
      "id": "q119",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "Oracle の BYOL では、Amazon EC2 Dedicated Host を使用することで物理コア数に基づくライセンス計算が可能です（仮想 vCPU 数ではなく物理コア数でカウント）。Dedicated Host はお客様専用の物理サーバーを占有するため、Oracle のハードパーティション（Hard Partitioning）要件を満たします。適切なコア数の Dedicated Host を選択することでライセンスコストを最適化できます。\n\nD: 多くの vCPU を持つインスタンスはライセンスコストを増加させます。Oracle ライセンスは使用するコア数に基づくため、必要最小限のコア数が重要です。\n\nB: ライセンス込みの RDS for Oracle は Oracle が管理するライセンスを使用するため、BYOL（既存ライセンスの持ち込み）には対応していません。既存のオンプレミスライセンスを活用できません。\n\nA: Spot インスタンスは中断リスクがあり、Oracle データベースのような本番ミッションクリティカルなワークロードには適していません。また Oracle の Hard Partitioning 要件も満たしません。"
    },
    {
      "id": "q122",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "IRSA（E）は Kubernetes ServiceAccount と IAM ロールを OIDC フェデレーションで紐付けることで、Pod レベルの最小権限 IAM アクセスを実現します。インスタンスプロファイルと異なり、同一ノード上の異なる Pod に異なる IAM 権限を付与できます。Amazon VPC CNI のネットワークポリシー機能または Calico（A）は Kubernetes の NetworkPolicy リソースを実装し、Pod 間通信のラベルセレクターベースの細かいアクセス制御を実現します。\n\nD: インスタンスプロファイルに全権限をまとめることは最小権限原則に反し、全 Pod が全リソースにアクセスできてしまいます。\n\nC: セキュリティグループは IP/ポートレベルの制御で、Kubernetes の Pod ラベルベースの動的なネットワークポリシーには対応していません（EKS の SG for Pods は使えますが、NetworkPolicy の代替には限定的）。\n\nB: 全 Pod に同一の広い権限を付与することは最小権限原則に大きく反します。"
    },
    {
      "id": "q125",
      "type": "single",
      ...
      "explanation": "Amazon MQ はマネージドのメッセージブローカーサービスで、ActiveMQ と RabbitMQ をサポートします。JMS プロトコルに完全対応しているため、既存の JMS クライアントコードを最小限の変更（接続先エンドポイントのみ変更）で Amazon MQ に接続できます。IBM MQ から JMS 互換のブローカーへの移行として最も自然なパスです。\n\nC: SQS は AWS 独自の API で、JMS のような標準プロトコルをネイティブサポートしておらず、アプリケーションコードの書き直しが必要です。\n\nB: Kinesis はストリーミングデータの高スループット取り込みに特化しており、JMS のリクエスト-レスポンスパターンや双方向メッセージングには設計されていません。\n\nA: EventBridge はイベント駆動アーキテクチャのルーティングに有効ですが、既存の JMS インターフェースとは互換性がなく、アプリケーションの大幅な書き直しが必要です。"
    },
    {
      "id": "q126",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "AWS Storage Gateway のファイルゲートウェイはオンプレミスに設置する仮想アプライアンス（または物理アプライアンス）で、NFS と SMB のファイルインターフェースを提供します。アプリケーションは既存の NFS プロトコルでアクセスし、ファイルゲートウェイがローカルキャッシュを管理しながらバックグラウンドで S3 にデータを保存・同期します。アプリケーションの変更なしにデータを AWS S3 に保存できます。\n\nC: アプリケーションの書き直しは「変更できない」という制約に反します。\n\nD: Amazon EFS は Direct Connect 経由でマウント可能ですが、NFS プロトコルを使ってオンプレミスから直接アクセスするには特別な設定が必要で、EFS のデータは S3 ではなく EFS に保存されます。S3 への保存要件を満たしません。\n\nB: Amazon S3 はオブジェクトストレージで、直接 NFS マウントする標準機能はありません（AWS が公開している NFS マウント機能は存在しません）。"
    },
    {
      "id": "q129",
      "type": "single",
    ...
    },
    {
      ...
      "choices": [
        "漏洩したアクセスキーを即座に AWS コンソールで無効化（または削除）し、IAM アクセスアドバイザーで不審なアクセスがなかったか確認する。CloudTrail で漏洩期間中の API アクティビティを調査する。将来の防止策として、AWS Secrets Manager と AWS IAM Identity Center（EC2 には IAM ロール）を使い、アクセスキーを廃止する。GitHub の Secret Scanning と Amazon Q Developer のコードレビューでコード内のシークレット検出を自動化する",
      ...
      "explanation": "漏洩したアクセスキーへの対応は多層的なアプローチが必要です。②即時無効化でリスクを排除、①CloudTrail での調査で被害範囲を特定、③IAM ロールへの移行でアクセスキー廃止、④自動スキャンツールで再発防止というプロセスが包括的な対応です。GitHubでのSecret Scanning（GitHub の機能）や Amazon Q Developer のコードレビューでコードのシークレット漏洩を事前に検出する仕組みも重要です。\n\nB: リポジトリを Private にするだけではアクセスキーはまだ有効であり、既に不正使用されている可能性があります。即時の無効化が最優先です。\n\nC: Git の歴史からシークレットを削除することは重要ですが、漏洩期間中の不正アクセスの調査と現在のキーの無効化を先に行う必要があります。また Git の歴史の書き換えは単純ではありません。\n\nD: IAM パスワードポリシーはコンソールサインインに関するもので、アクセスキーのローテーションには直接対応していません。"
    },
    {
      "id": "q131",
      "type": "single",
      ...
      "explanation": "Application Discovery Service が収集したネットワーク接続データを Migration Hub に統合すると、サーバー間の依存関係グラフが可視化されます。Migration Hub のアプリケーショングループ機能を使って依存するサーバーをグループ化し、各グループを Wave に割り当てることで、依存関係が崩れない順序での移行計画が立てられます。\n\nB: 手動グループ作成は可能ですが、依存関係データが自動収集されているならそれを活用するべきです。大量のサーバーを手動グループ化するのは非効率です。\n\nD: Excel での手動作成は正確性が低く、大量のサーバーでは現実的ではありません。Discovery Service の自動収集データを活用する方が効率的です。\n\nA: 依存関係を事前に把握せずに移行することは、本番環境での障害リスクを大幅に高めます。計画フェーズでの依存関係分析は必須です。"
    },
    {
      "id": "q132",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "Route 53 のレイテンシーベースルーティングは、DNS クエリの発信元から各リージョンへのレイテンシーを測定し、最も低レイテンシーのリージョンに自動転送します。ヘルスチェックを組み合わせると、障害リージョンは DNS 応答から自動除外され、正常なリージョンにフェイルオーバーします。TTL を短く（30〜60秒）設定することで障害時の切り替え時間を短縮できます。\n\nC: シンプルルーティングは単一エンドポイントのみで、マルチリージョンへのルーティングやフェイルオーバーに対応していません。\n\nD: CloudFront のオリジングループフェイルオーバーは有効ですが、CloudFront はコンテンツキャッシュが主目的であり、動的 API のマルチリージョン分散にはRoute 53 レイテンシーベースルーティングの方が直接的です。\n\nA: ハードコードされた IP と手動リージョン選択は自動フェイルオーバーに対応しておらず、ユーザー体験も低下します。"
    },
    {
      "id": "q137",
      "type": "single",
      ...
      "explanation": "ECS タスク定義の secrets フィールドで Secrets Manager または SSM Parameter Store の ARN を参照することで、ECS がタスク起動時に自動的にシークレットを取得してコンテナに注入します。シークレットはタスク定義には保存されず、IAM ロールによるアクセス制御と Secrets Manager の自動ローテーション機能も活用できます。\n\nB: Base64 エンコードは暗号化ではなくエンコードであり、容易にデコードできます。セキュリティの向上にはなりません。\n\nA: S3 からの取得も機能しますが、起動スクリプトのカスタム実装が必要で、ECS ネイティブの secrets 統合より管理が複雑です。また S3 のアクセスポリシーとライフサイクル管理も別途必要です。\n\nC: EFS へのファイル保存は可能ですが、ECS の secrets フィールドのネイティブ統合より複雑で、Secrets Manager の自動ローテーション機能も利用できません。"
    },
    {
      "id": "q138",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "Repurchase（C）は既存のシステムを廃止して SaaS 製品（Workday 等）に移行する戦略です。クラウド版の SaaS が存在する場合、カスタム開発・運用のコストを SaaS の利用コストと比較して Repurchase が最もコスト効率的なことが多いです。Retain（D）は現時点では移行せずに現状維持する戦略で、今後 2 年で再評価する予定のシステムに適しています。\n\nB: Rehost は既存システムをそのまま EC2 に移行する戦略で、クラウド版 SaaS が存在するケースではより多くの価値を生む Repurchase の方が適しています。\n\nA: Retire は使用されていないまたは不要なシステムを廃止する戦略です。2 年間維持する予定のシステムには適しません。\n\nE: Refactor はクラウドネイティブに再設計する最も労力のかかる戦略で、既に SaaS が存在する HR システムに適用する必要はありません。"
    },
    {
      "id": "q142",
      "type": "single",
      ...
      "explanation": "AWS Glue は JDBC コネクターで Teradata など多くのデータウェアハウスに接続できます。Glue ETL ジョブをスケジュール実行（EventBridge またはトリガー）で毎日自動化し、ジョブブックマーク機能で前回の実行以降に更新されたデータのみを抽出する増分ロードが可能です。Direct Connect 経由でのオンプレミス接続にも対応しています。\n\nD: 手動エクスポートと SFTP アップロードは自動化されておらず、運用負荷が高くなります。\n\nB: Kinesis Data Firehose は AWS エンドポイントへのデータ取り込みに使いますが、オンプレミスから Direct Connect 経由で Firehose をトリガーするには別途エージェントや Lambda が必要で、バッチデータには Glue の方が適しています。\n\nA: DMS の継続的レプリケーションは MySQL/Oracle/PostgreSQL 等の主要 RDBMS でサポートが充実していますが、Teradata からの CDC（変更データキャプチャ）は制限が多く、毎日のバッチ取り込みには Glue の方が信頼性が高いです。"
    },
    {
      "id": "q143",
      "type": "single",
      ...
      "explanation": "Amazon RDS Performance Insights はリアルタイムおよび過去 7 日間（デフォルト）のデータベース負荷を可視化します。DBLoad（データベースが処理しているアクティブセッション数）、待機イベント（どのリソースを待っているか）、トップ SQL（最も負荷の高いクエリ）を GUI でドリルダウン分析できます。追加のソフトウェアなしに数クリックで有効化でき、RDS の大幅な設定変更も不要です。\n\nD: CloudWatch のインスタンスメトリクスはリソース使用率の確認には有効ですが、どのクエリが原因かを特定する SQL レベルの分析はできません。\n\nB: スロークエリログの有効化は有効ですが、log_min_duration_statement パラメーターの設定が必要で、ログファイルの手動ダウンロードと分析の手間があります。Performance Insights の方がはるかにシンプルです。\n\nA: スナップショットはバックアップ目的であり、クエリパフォーマンス分析には一切使えません。"
    },
    {
      "id": "q144",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "EC2 Auto Scaling グループへのリフト＆シフトが「最小限の変更」要件に最も適しています。MGN でオンプレミス VM をほぼそのまま EC2 に移行し、カスタム AMI として Auto Scaling グループに登録します。既存の Apache 設定ファイル（バーチャルホスト、.htaccess）もそのまま引き継げます。ALB と Auto Scaling の組み合わせでスケーリングとコスト最適化を実現します。\n\nB: Lambda への移行は Apache/Tomcat のアプリケーションを全面書き直しする必要があり「最小限の変更」要件に大きく反します。\n\nD: Elastic Beanstalk は Tomcat WAR デプロイをサポートしますが、Apache のバーチャルホスト設定や .htaccess をそのまま使うには追加設定が必要で、EC2 への直接移行よりも変更が多くなります。\n\nA: 静的ホスティングへの移行は動的コンテンツ（Tomcat）の処理方法の大幅な変更が必要で「最小限の変更」要件に反します。"
    },
    {
      "id": "q147",
      "type": "single",
    ...
    },
    {
      ...
      "explanation": "API Gateway の使用量プランはクライアント種別ごとに異なるスロットリング（RPS）とクォータ（月間リクエスト数）を定義し、API キーに紐付けることで細かいアクセス制御が実現できます。CloudWatch で API キー別の使用量メトリクスを確認できます。内部サービスはレート制限なし（上限なし）の使用量プランを作成することで対応できます。\n\nD: 同一 API キーでは個別クライアントの制御が不可能で、IP アドレスによる判別は NAT ゲートウェイや CDN を経由する場合に機能しません。\n\nA: AWS WAF のレート制限は IP アドレスベースで全体的な DDoS 対策に使うものであり、クライアント種別ごとの細かいレート制限と使用量監視には対応していません。\n\nB: 別ステージへの分割は API の管理を複雑にし、同じ API リソースを複数回維持する必要があります。使用量プランの方がはるかにシンプルです。"
    },
    {
      "id": "q150",
      "type": "single",
