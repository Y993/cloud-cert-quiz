...
window.CERT_ARCH.gcp = {
  ...
  links: {
    ...
    "gke>gcs":         { t: "iam", l: "Workload Identity", d: "Pod の Kubernetes サービスアカウントを Workload Identity で IAM サービスアカウントに紐づけ、storage.objectViewer 等のロールでアクセスする。SA キーファイルを Secret に配らないのが現行のベストプラクティス。" },
    ...
    "cloudrun>bigquery":      { t: "iam", l: "クエリ実行", d: "Cloud Run のサービスアカウントに BigQuery のジョブ実行・データ参照ロールを付けてクエリする。API キーを埋め込まず SA の権限で叩くのが基本。重い集計はバッチに寄せてスロット消費を平準化する。" },
