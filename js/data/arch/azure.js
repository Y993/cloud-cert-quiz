...
window.CERT_ARCH.azure = {
  ...
  links: {
    ...
    "appservice>cosmos":   { t: "iam", l: "マネージドID接続", d: "接続文字列のキーを捨て、マネージド ID + Cosmos DB の RBAC でデータプレーンに繋ぐのが現行推奨。閉域化するならプライベートエンドポイントを併用する。" },
    ...
    "aks>cosmos":    { t: "iam", l: "Workload Identity", d: "Pod を Microsoft Entra Workload ID で AKS 上から Cosmos DB の RBAC に紐づけてアクセスする。接続キーを Secret に配らず、トークンベースで繋ぐのが Azure の現行流儀。" },
