...
window.CERT_ARCH.aws = {
  ...
  links: {
    ...
    "user>s3":          { t: "net", l: "静的配信/直DL", d: "S3 の静的ウェブサイトホスティングや署名付き URL でコンテンツを直接配信・ダウンロードさせる最小構成。本番は手前に CloudFront + OAC を置き、バケットは非公開にして直アクセスを塞ぐのが定石。" },
    "user>ec2":         { t: "net", l: "パブリックIP直アクセス", d: "EC2 にパブリック IP を付けて直接アクセスする最小構成。実運用では手前に ALB（必要なら CloudFront + WAF）を置き、EC2 はプライベートサブネットに隠す。SSH/RDP は踏み台より Session Manager 経由が安全。" },
    ...
    "ec2>lambda":         { t: "iam", l: "SDKでInvoke", d: "EC2 上のアプリから SDK で Lambda を呼び出す。インスタンスプロファイルの IAM ロールに lambda:InvokeFunction を付ける。重い処理や非同期処理を関数側に切り出したいときに使う。" },
